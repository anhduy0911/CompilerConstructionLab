...
void compileAssignSt(void)
{
  ...
  Type *lValType = NULL;
  Type *expType = NULL;
  lValType = compileLValue();
  eat(SB_ASSIGN);
  expType = compileExpression();
  checkTypeEquality(lValType, expType);
}
...
void compileForSt(void)
{
  ...
  eat(SB_ASSIGN);
  Type *expType1 = compileExpression();
  checkBasicType(expType1);

  eat(KW_TO);
  Type *expType2 = compileExpression();
  checkBasicType(expType2);

  checkTypeEquality(expType1, expType2);
  checkTypeEquality(var->varAttrs->type, expType1);
...
void compileArgument(Object *param)
{
  ...
  if (param->paramAttrs->kind == PARAM_REFERENCE)
  {
    ...
    // use this to report that it's expecting an ident, not an expression
    argumentType = compileLValue();
  }
  else
  {
